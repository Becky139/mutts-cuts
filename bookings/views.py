...
def create_booking(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            check_existing_bookings = Booking.objects.filter(date__date=req_date, start_time=req_time)
            # logic - only 1 appointment can be taken at a given time as only one staff member present
            if check_existing_bookings >= 1:
                return redirect('view_booking')
                messages.add_message(request, messages.ERROR(request, message), "No appointmnet available for this time")
            else:
